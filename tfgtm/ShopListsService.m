...
@implementation ShopListsService


+ (ShopListsService *)defaultService
{
    // Create a singleton instance of ShopListsService
    static ShopListsService* service;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        service = [[ShopListsService alloc] init];
    });
    
    return service;
}

-(ShopListsService *)init
{
    self = [super init];
...
-(void)pullData:(QSCompletionBlock)completion
{
    ...
    [self.syncTable pullWithQuery:query queryId:@"allShopLists" completion:^(NSError *error) {
        [self logErrorIfNotNil:error];
